...
# global parameters: dimension
if (!file.exists("./predictions/prediction_monthly_2019.csv")) {
  ...
  colnames(results) <- c("year", "month", "name", "crop", 
                        ...
                        "prediction_upper_95")
  write.csv(results, "./predictions/prediction_monthly_2019.csv", row.names = FALSE)
}


args <- as.double(commandArgs(TRUE))
test.month.beginning <- args[1]
test.month.end <- args[2]

loginfo("Overwriting existing prediction file")
results <- read.csv("./predictions/prediction_monthly_2019.csv")
...
for (crop in crops)
{
  ...
  ndvi.test <- read.csv("../sweden_regional_yield_monthly_model_test.csv")
  noaa.test <- read.csv("../climatic_sweden_regional_yield_monthly_model_test.csv")
  ...
  for (test.month in test.month.beginning:test.month.end)
  {
    ...
    plot(c(1:6), predictions[[2]], ylim= c(0,8000), main="2019 Spring Barley RMSE Training: %s"%--%model$rmse_train)
    ...
    jpeg("../saved/%s/plots/importance_monthly_%s-%s.jpg" %--% c(crop, test.month, test.years) ,width = 1000, height = 1000)
    ...
    write.csv(drivers, "../saved/%s/plots/importance_monthly_%s-%s.csv" %--% c(crop, test.month, test.years), row.names = FALSE)
  }
}
